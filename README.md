...
---
# 📦 JuniorSir APT Repository (Termux)

Official APT repository for installing **gana** and future packages directly inside Termux.

Hosted via GitHub Pages.

---

## 🚀 Installation (Recommended – Signed & Secure)

### 1️⃣ Add Repository GPG Key

```bash
apt install gnupg
mkdir -p $PREFIX/etc/apt/sources.list.d
curl -fsSL https://juniorsir.github.io/gana/public.key | gpg --dearmor > gana.gpg
mv gana.gpg $PREFIX/etc/apt/trusted.gpg.d/
```

---

### 2️⃣ Add Repository Source
...
- **InRelease / Release.gpg** → Signed verification
- **public.key** → GPG key for trust
...
- ✔ GPG signed
- ✔ No `trusted=yes`
- ✔ Clean APT integration

All packages are signed before publishing.
...
```bash
rm $PREFIX/etc/apt/sources.list.d/gana.list
rm $PREFIX/etc/apt/trusted.gpg.d/gana.gpg
pkg update
```
...
GitHub: https://github.com/juniorsir

---

# 📜 License

Packages inside this repository follow their respective licenses.
...
- Hosted via GitHub Pages
- Built using `dpkg-scanpackages` + `apt-ftparchive`
- Fully compatible with standard APT workflow
